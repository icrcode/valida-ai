...
const TITULOS_ETAPA: Record<Etapa, string> = {
  1: 'Etapa 1 de 4 — Seus dados',
  2: 'Etapa 2 de 4 — Dados acadêmicos',
  3: 'Etapa 3 de 4 — E-mail institucional',
  4: 'Etapa 4 de 4 — Crie sua senha',
};
...
function ErroAlerta({ mensagem }: { mensagem: string }) {
...
export function Cadastro() {
  ...
  // Universidades disponíveis (derivadas dos cursos ativos)
  ...
  // Cursos da universidade selecionada, agrupados por nome + modalidade
  ...
  // Turnos disponíveis para o curso selecionado
  ...
  // Domínios de e-mail aceitos pela universidade selecionada
  ...
  function validarEtapa3(): string | null {
  ...
  function validarEtapa4(): string | null {
  ...
  function avancar(e: FormEvent, validador: () => string | null, proxima: Etapa) {
    e.preventDefault();
    const erroValidacao = validador();
    if (erroValidacao) { setErro(erroValidacao); return; }
    setErro('');
    setEtapa(proxima);
  }

  function voltar(anterior: Etapa) {
    setErro('');
    setEtapa(anterior);
  }
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const erroValidacao = validarEtapa4();
    if (erroValidacao) { setErro(erroValidacao); return; }
    setErro('');
    setLoading(true);
    try {
  ...
  const botaoPrimarioCls = 'flex-1 flex items-center justify-center gap-2 rounded-lg bg-[#618C7C] px-4 py-2.5 text-sm font-medium text-white transition-all hover:bg-[#7AAA9A] hover:shadow-[0_0_24px_rgba(97,140,124,0.3)] focus:outline-none focus:ring-2 focus:ring-[#618C7C] focus:ring-offset-2 focus:ring-offset-[#011140] disabled:opacity-50';

  const botaoVoltarCls = 'flex-1 rounded-lg border border-white/10 bg-transparent px-4 py-2.5 text-sm font-medium text-white/70 transition-all hover:border-white/20 hover:bg-white/5 hover:text-white focus:outline-none focus:ring-2 focus:ring-white/20 focus:ring-offset-2 focus:ring-offset-[#011140] disabled:opacity-50';
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#010A26] px-4 py-8">
      <div className="relative w-full max-w-sm animate-fade-up">
        ...
        <div className="rounded-2xl border border-white/8 bg-[#011140] p-8 shadow-2xl shadow-black/40">
          <h2 className="mb-1 text-center text-base font-semibold text-white/80">Criar conta de estudante</h2>
          <p className="mb-4 text-center text-xs text-white/40">{TITULOS_ETAPA[etapa]}</p>

          {/* Indicador de etapas */}
          <div className="mb-6 flex items-center justify-center gap-2" aria-hidden="true">
            {([1, 2, 3, 4] as const).map((n) => (
              <span key={n} className={`h-1.5 rounded-full transition-all ${etapa === n ? 'w-6 bg-[#618C7C]' : 'w-1.5 bg-white/15'}`} />
            ))}
          </div>

          {etapa === 1 && (
            <form onSubmit={(e) => avancar(e, validarEtapa1, 2)} className="flex flex-col gap-4 animate-fade-up">
              ...
              <ErroAlerta mensagem={erro} />

              <button type="submit" className={`mt-1 ${botaoPrimarioCls}`}>
                Continuar
              </button>
            </form>
          )}
          ...
          {etapa === 2 && (
            <form onSubmit={(e) => avancar(e, validarEtapa2, 3)} className="flex flex-col gap-4 animate-fade-up">
              <div className="flex flex-col gap-1.5">
                ...
                {loadingCursos ? (
                  ...
                  <select id="universidade" value={universidadeId} autoFocus
              ...
              <div className="mt-1 flex gap-3">
                <button type="button" onClick={() => voltar(1)} className={botaoVoltarCls}>Voltar</button>
                <button type="submit" className={botaoPrimarioCls}>Continuar</button>
              </div>
            </form>
          )}

          {etapa === 3 && (
            <form onSubmit={(e) => avancar(e, validarEtapa3, 4)} className="flex flex-col gap-4 animate-fade-up">
              <div className="flex flex-col gap-1.5">
              ...
              <div className="mt-1 flex gap-3">
                <button type="button" onClick={() => voltar(2)} className={botaoVoltarCls}>Voltar</button>
                <button type="submit" className={botaoPrimarioCls}>Continuar</button>
              </div>
            </form>
          )}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 animate-fade-up">
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <input id="senha" type={mostrarSenha ? 'text' : 'password'} value={senha} autoFocus
              ...
              <div className="mt-1 flex gap-3">
                <button type="button" onClick={() => voltar(3)} disabled={loading} className={botaoVoltarCls}>Voltar</button>
                <button type="submit" disabled={loading} className={botaoPrimarioCls}>
